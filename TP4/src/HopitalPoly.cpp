...
set<shared_ptr<Medecin>, ComparerPersonne<Medecin>> HopitalPoly::getMedecinParCritere(
    function<bool(const shared_ptr<Medecin>&)> critere) const
{
    set<shared_ptr<Medecin>, ComparerPersonne<Medecin>> set;
    for (const auto& employee : employes_) {
        if (auto medecin = dynamic_pointer_cast<Medecin>(employee)) { 
            if (critere(medecin)) {
                set.insert(medecin);
            }
        }
...
{
    auto it = find_if(employes_.begin(), employes_.end(),
        [&](const shared_ptr<Employe>& emp) { return critere(*emp); });
...
void HopitalPoly::afficherEmploye(ostream& out) const {
    std::for_each(employes_.begin(), employes_.end(),
        [&out](const std::shared_ptr<Employe>& employe) {
